    ...
    public class CITask : Abstractions.CITask, IDisposable
    {
        ...
        public Task<bool> RunAsync(string Filename, string Arguments = null,int TimeLimit = 0, string WorkingDirectory = null, IDictionary<string, string> AdditionalEnvironmentVariables = null)
        {
            return Task.Factory.StartNew(() =>
            {
                ...
                {
                    Node.Output(new Output { TaskId = this.Id, Time = DateTime.Now, Type = OutputType.Successful, Text = $"The process exited with the code 0." });
                    process.Dispose();
                    Processes.Remove(process);
                    return true;
                }
                ...
                {
                    Node.Output(new Output { TaskId = this.Id, Time = DateTime.Now, Type = OutputType.Successful, Text = $"The process exited with the code {process.ExitCode}." });
                    process.Dispose();
                    Processes.Remove(process);
                    return false;
                }
